...
const FlashcardDetails = () => {
  ...
  const displayCard = (id) => {
    const showSingleCard = myCard.cards.filter((c) => c.cardid === id);
    setFlashcardDetail(showSingleCard[0]);
  };
  ...
  useEffect(() => {
    myCard.cards && setFlashcardDetail(myCard.cards[0]);
  }, [myCard]);
    ...
    <div className="flex flex-col text-slate-600">
      <div className="flex">
        ...
        <div className="flex flex-col">
          <h2 className="text-xl text-black font-bold">{myCard.groupname}</h2>
          {myCard.groupdescription && (
            <p className=" my-2">{myCard.groupdescription}</p>
          )}
        </div>
      </div>
      <div className="mt-6 grid grid-rows-1 md:grid-cols-4">
        <div className="col-span-1 bg-white w-[60vw] md:w-[10rem] xl:w-[17rem] m-5 px-1 py-2 h-fit rounded-md">
          <h2 className="p-2 ">Flashcards</h2>
          <hr />
          <hr className="mb-2" />
          {myCard.cards &&
            myCard.cards.map((card) => (
              <p
                key={card.cardid}
                className={`py-2 px-8 text-slate-700 font-medium hover:bg-slate-100 cursor-pointer ${
                  card.cardid === flashcardDetail.cardid &&
        ...
        <section className="col-span-3 md:col-span-2 flex flex-col xl:flex-row items-center w-full bg-white shadow-lg rounded-lg">
          ...
          <p className={`w-full p-6 py-10 `}>
            {flashcardDetail.carddescription}
          </p>
        </section>
        <div className="col-span-1 hidden md:flex flex-col items-center space-y-5">
          <button
            type="button"
            onClick={openModal}
